...
ggplot(agg_exposure_dat %>% filter(year == 2024 & month(date) >= 6), 
  ...
  scale_color_brewer(name = "Exposed group") +
  ylim(0, 16) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, vjust = 0.75),
...
ggplot(dat %>% filter(year == 2024 & month(date) >= 6), 
  ...
  facet_wrap(~zip3) +
  labs(x = "Date", y = "Mean precipitation [mm]") +
  scale_color_manual(values = c("Flooded" = "#4B637B",
                                "Non-flooded" = "#5C3B25"),
                     name = "Exposure group during Hurricane Helene") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, vjust = 0.75),
        legend.position = "bottom",
...
ggplot(agg_dat %>% filter(year %in% c(2017:2024)), 
  ...
  scale_color_manual(values = MetBrewer::met.brewer(name = "Egypt", n = 8),
                     name = "") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, vjust = 0.75),
        legend.position = "bottom")
...
<<<<<<< HEAD
  ...
  ungroup() %>%
  mutate(case_rate_per10k = sum_events/sum_pop*1e4,
         inundation_exposure_plot = ifelse(inundation_exposure, "Flooded: multiply imputed", 
                                           "Non-flooded: multiply imputed"))

median_agg_exposure_masked_dat <- agg_exposure_masked_dat %>%
  group_by(date, year, inundation_exposure_plot) %>%
  summarise(sum_events = median(sum_events),
            hurricane_3week = unique(hurricane_3week),
            hurricane_5week = unique(hurricane_5week),
            hurricane_8week = unique(hurricane_8week),
            case_rate_per10k = median(case_rate_per10k)) %>%
  ungroup()

ggplot(median_agg_exposure_masked_dat %>% filter(year == 2024 & month(date) >= 6), 
       aes(x = date, y = case_rate_per10k, color = inundation_exposure_plot)) +
  geom_rect(aes(xmin = as.Date('2024-09-27'),
                xmax = as.Date("2024-11-15"),
                ymin = -Inf,
  ...
  geom_rect(aes(xmin = as.Date('2024-09-27'),
                ...
                ymin = -Inf,
                ymax = Inf), fill = 'darkgrey', color = "darkgrey", alpha = 0.5) +
  geom_text(x = as.Date('2024-10-04'), y = 4.4, label = "3 weeks", size = 2.75,
             color = "black", fontface = "bold") +
  geom_text(x = as.Date('2024-10-18'), y = 4.4, label = "5 weeks", size = 2.75,
             color = "black", fontface = "bold") +
  geom_text(x = as.Date('2024-11-05'), y = 4.4, label = "8 weeks", size = 2.75,
             color = "black", fontface = "bold") +
  geom_point(shape = 8) +
  geom_line(linetype = "dashed") +
  geom_point(data = agg_exposure_dat %>% filter(year == 2024 & month(date) >= 6), 
             aes(x = date, y = case_rate_per10k, color = inundation_exposure_plot)) +
  geom_line(data = agg_exposure_dat %>% filter(year == 2024 & month(date) >= 6), 
             aes(x = date, y = case_rate_per10k, color = inundation_exposure_plot)) +
  labs(x = "Date", y = "AGI visits per 10k") +
  scale_color_manual(values = c("Flooded" = "#4B637B", 
                                "Flooded: multiply imputed" = "#4B637B",
                                "Non-flooded" = "#5C3B25", 
                                "Non-flooded: multiply imputed" = "#5C3B25")) +
